...
function getSamplesCatalogUiText() {
  const root = document.getElementById('samplesCatalogUiText');
  const totalTemplate = getTextToken(root, 'totalTemplate', 'মোট {total} টি স্যাম্পল দেখানো হচ্ছে');
  ...
  return {
    empty: getTextToken(root, 'empty', 'কোনো স্যাম্পল পাওয়া যায়নি'),
...
export function createSamplesCatalogFilters({
  ...
  const uiText = getSamplesCatalogUiText();
