...
function updateSampleCount() {
  ...
  const visibleCards = getSampleCards().filter(card => card.style.display !== "none");
...
function saveSample() {
  closeMo("moSample");
  toast("success", "Sample saved", "Static preview only");
}
...
window.filtSamples = filtSamples;
window.openSampleMo = openSampleMo;
window.saveSample = saveSample;
window.syncClr = syncClr;
window.autoSID = autoSID;
window.updSPrev = updSPrev;
